...
> - PS：从某台设备出发，经过一个个路由，汇集到运营商（电信、移动等）管理的路由（运营商会有一个网络管理队伍，制定好详细的路由规则），经过这个路由后，就能正确地跳到其他路由。

# 链路层
> ## ARP协议：解析IP与网卡硬件地址的映射（局域网）：【【ARP】【IP】【TCP】【data】】
> ![alt 链路表](src/picture/arp.jpeg)  
> 通过网关地址，转换为网卡硬件地址，向下层传递
> - ARP（address resolution protocol），主机发送信息时会将包含目标IP的ARP请求广播到局域网的所有主机，并接收返回信息，并缓存起来。
